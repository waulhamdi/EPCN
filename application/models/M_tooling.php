...
class M_tooling extends CI_Model {
  ...
  ///@seecari_inisiator_responsible
    ...
    // ///@see supplier_name_and_code()
    //  ///@note untuk select filter
    //  ///@attention 
    // public function supplier_name_and_code()
    // {
    //     $query =  $this->db->get('tb_tooling2')->result(); //contoh dari tabel input problem nanti ganti table kalau sudah fiks
    //     return $query;
    // }
    ...
    public function get_report_no($product_name)
    {
     ...
     ///@see get_tooling_Registrasi()
     ///@note untuk pcn register mendapatkan no pcn
     ///@attention 
     public function get_tooling_Registrasi()
     {
         // return $this->db->get_where('tb_product', ['hdrid' => $product_id])->row_array();
          $query = $this->db->query("select * from tb_tooling ");
         if ($query->num_rows() > 0) {
             $query = $query->row();
         }else{
             $query = (object) array(
    ...
    public function getdatabysession()
    {
        ...
        ///@see number_of_data_per_month_toolingLIST()
     ///@note fungsi digunakan menarik data pcnlist ke table tiap bulan
     ///@attention 
    public function number_of_data_per_month_toolingLIST($year, $month){
        $query = $this->db->query("SELECT COUNT(*) num FROM tb_toolingLIST WHERE YEAR(transaction_date) = '".$year."' AND MONTH(transaction_date) = '".$month."' GROUP BY  MONTH(transaction_date)");
        if ($query->num_rows() > 0) {
            $query = $query->row();
            return $query->num;
        }else{
    ...
    public function upcoming_5_month_name(){
     ...
     ///@see last_5_month_toolingLIST()
     ///@note fungsi digunakan menarik data pcnlist ke table tiap 5 bulan terakhir
     ///@attention 
    public function last_5_month_toolingLIST(){
        $month = date('m');
        $year = date('Y');
        $datas = array();
        for ($i=0; $i < 5; $i++) {
            if ($month > 0) {
                array_push($datas, $this->number_of_data_per_month_toolingLIST($year, $month--));
            } else {
                $month = 12;
                array_push($datas, $this->number_of_data_per_month_toolingLIST(--$year, $month--));
            }
     ...
     ///@note fungsi digunakan menarik data pcnlist ke table tiap 5 bulan kedepan
     ///@attention 
    public function upcoming_5_month_toolingLIST(){
        $month = date('m');
        $year = date('Y');
        $datas = array();
        for ($i=0; $i < 5; $i++) {
   ...
   ///@see number_of_data_per_year_toolingLIST()
     ///@note fungsi digunakan menarik data pcnlist ke table tiap tahun
     ///@attention 
    public function number_of_data_per_year_toolingLIST($year){
        $query = $this->db->query("SELECT COUNT(*) num FROM tb_toolingLIST WHERE YEAR(transaction_date) = '".$year."' GROUP BY  YEAR(transaction_date)");
        if ($query->num_rows() > 0) {
            $query = $query->row();
            return $query->num;
        }else{
    ...
    public function upcoming_5_years(){
        ...
        return $datas; 
         ...
         ///@see last_5_years_toolingLIST()
     ///@note fungsi digunakan menarik data pcnlist ke table tiap 5 tahun terakhir
     ///@attention 
    public function last_5_years_toolingLIST(){
        $year = date('Y');
        $datas = array();
        for ($i=0; $i < 5; $i++) {
            array_push($datas, $this->number_of_data_per_year_toolingLIST($year--));
        }
        return array_reverse($datas);
    }

    ///@see upcoming_5_years_toolingLIST()
     ///@note fungsi digunakan menarik data pcnlist ke table tiap 5 tahun kedepan
     ///@attention 
    public function upcoming_5_years_toolingLIST(){
        $year = date('Y');
        $datas = array();
        for ($i=0; $i < 5; $i++) {
            array_push($datas, $this->number_of_data_per_year_toolingLIST($year++));
    ...
    public function data_per_supplier(){
        $supplier = $this->db->query("SELECT nama_supplier FROM tb_toolingLIST WHERE nama_supplier IS NOT NULL GROUP BY  nama_supplier");
        $supplier_array = array();
        for ($i=0; $i < $supplier->num_rows(); $i++) {
            array_push($supplier_array, $supplier->row($i)->nama_supplier);
        }
        $num = $this->db->query("SELECT COUNT(*) num FROM tb_toolingLIST WHERE nama_supplier IS NOT NULL GROUP BY  nama_supplier");
        $num_array = array();
