...
class M_setting_approval extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM setting_approval_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from setting_approval_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('setting_approval_approver',$data);
...
//         $DB2->select('user_name,name,office_email');         
...
//         $DB2 = $this->load->database('db_central_user', TRUE);
//         $DB2->select('department_code,department_name');         
//         $query = $DB2->get('Tb_department')->result();  
//         $DB2->Close(); 
//         return  $query;
...
//    }
   ...
   public function Tampil_nik()
   {
        ...
        // $query =  $this->db->get('Tb_user_login')->result(); 
        // return $query;
   ...
   public function Tampil_position()
   {
    $query =  $this->db->get('tb_position')->result(); 
    return $query;
   }
   ...
   public function Getuser($nik)
   {
        ...
        //Cari nama dan email dari database db_central_user
        
        $DB2 = $this->load->database('db_central_user', TRUE);   // Database central user   

        $query =$DB2->query("select top 1 user_name,name,office_email from Tb_user_login where user_name='$nik' ");
