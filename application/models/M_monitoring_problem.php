...
class M_monitoring_problem extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM monitoring_problem_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from monitoring_problem_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('monitoring_problem_approver',$data);
   ...
   // Get Data Group monitoring_problem
   public function get_group_monitoring_problem()
   {
    $query =  $this->db->get('tb_group_monitoring_problem')->result();
    return $query;
   }
