...
class M_delegation extends CI_Model {
   ...
   function ajax_getbytrxid($hdrid,$table){      
      return $this->db->get_where($table, array('trxid' => $hdrid));
   }
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM delegation_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from delegation_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('delegation_approver',$data);
   ...
   public function Send_mail_reject($hdrid)
   {
    ...
    function Update_Data_Approver($problem_id,$nik)
    {
     ...
     function Update_Data_to($problem_id,$nik)
    {
 
        // Simpan responsible approver 1
        $this->tb_approval_update_to($problem_id,$nik,'4','');
        
                
    }


      function update_status_transaction($hdrid)
    {

        $query = $this->db->query("Update tb_input_problem set status_transaction=
            isnull((select top 1 ('Waiting ' + name + ' (' + position_name + ')' )status_transaksi from tb_approval 
            where problem_id='$hdrid' and date_approve is null),'Done')
            where hdrid='$hdrid' ");

    }

     function tb_approval_update_to($problem_id,$sequence)
    {


    //         nik2	name2	section2	email2
    //    DM1902222	Adrian Ramond Lubis	SUPPLIER DEVELOPMENT 	adrian.ramond.lubis.a2e@ap.denso.com


       $query = $this->db->query("
        update tb_input_problem
        set nik2=isnull((select top 1 nik_superior$sup from tb_superior where nik='$nik'),nik)
        ,name2=isnull((select top 1 name_superior$sup from tb_superior where nik='$nik'),name)
        ,section2=isnull((select top 1 email_superior$sup from tb_superior where nik='$nik'),office_email)
        ,email2=isnull((select top 1 kode_setion_superior$sup from tb_superior where nik='$nik'),department_code)        
   ...
   public function Tampil_user()
    ...
    // public function Tampil_problem()
    // {

    //     $this->db->select('hdrid,problem');
    //     $this->db->from('tb_input_problem');
    //     return $this->db->get()->result();

    // }

    public function Tampil_pcn_no()
    {

        $this->db->select('hdrid ,supplier_name');
        $this->db->from('tb_PCN');
        return $this->db->get()->result();
