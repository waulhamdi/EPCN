...
class M_PCNLIST extends CI_Model {
    
    
    ///@see get tables
     ///@note fungsi data web bisa masuk ke database
     ///@attention jika data web masuk ke web tapi tidak masu ke database itu salah di databasenya
   function get_tables($tables,$cari,$iswhere)
    ...
    ///@see max data
     ...
     ///@attention
      ...
      {
          $this->db->select_max('no_dokumen');//table auto increnete
          $this->db->like('no_dokumen', $mdate);//tanggal dinput
    ...
    ///@see max data
     ///@note fungsi digunakan menentukan data maximal
     ///@attention
      public function Max_data_pcn($mdate, $table)
      {
   ...
   function ajax_getbyno_dokumen($no_dokumen,$table){     
    ...
    ///@note fungsi digunakan untuk auto increnete 
    ///@attention nomor increnete sudah diinput otomatis akan terganti jadi tidak bisa diubah
   function ajax_getbyno_hdrid($hdrid,$table){     
...
//    }
   ...
   //@see send mail
    ...
    ///@attention 
   public function Send_mail($no_dokumen)
   {
    ...
    $query =$this->db->query("
     ...
     where no_dokumen='WTR2010009' and level_approve=(select min(level_approve) from department_approver where no_dokumen='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       $where = array('no_dokumen' => $query->no_dokumen); 
      ...
      $this->db->query("CALL sp_send_mail @no_dokumen='$query->no_dokumen',@nik='$query->nik_approver',@name='$query->name_approver',@email='widodo.a5v@ap.denso.com',@EmailSubject='Waiting Approver Wire transfer No ',@Email_body_content='You need approver'");
     ...
     $this->db->query("exec sp_send_mail @no_dokumen='123',@nik='DM1902060',@name='Widodo',@email='widodo.a5v@ap.denso.com',@EmailSubject='Finish Approver Wire transfer No ',@Email_body_content='You need approver'");
    ...
    ///@attention
   public function Send_mail_reject($no_dokumen)
   {
    
      // Cari data wire
      $query =$this->db->query("select no_dokumen,nik,name_requester,isnull(reject_by,'')reject_by,isnull(reason_reject,'')reason_reject from tb_wiretransfer where no_dokumen='$no_dokumen' ");
      $query=$query->row();
      ...
      if ($query2->num_rows()>0) {
          $query2=$query2->row();
         //  Kirim email
          $result =$this->db->query("exec sp_send_mail @no_dokumen='$query->no_dokumen',@nik='$query->nik',@name='$query->name_requester',@email='widodo.a5v@ap.denso.com',@EmailSubject='Rejected Wire transfer No ',@Email_body_content='Rejected by <b> $query->reject_by </b>',@comment='With reason = <b> $query->reason_reject </b>'")->result_array();
      }
