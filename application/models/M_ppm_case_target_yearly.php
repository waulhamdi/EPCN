...
class M_ppm_case_target_yearly extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM ppm_case_target_yearly_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from ppm_case_target_yearly_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('ppm_case_target_yearly_approver',$data);
...
//    public function get_ppm_case_date(){
//     $sql="UPDATE tb_ppm_case_target_yearly set ppm_case_date = Concat((DATEPART(YYYY, year_ppm_case_target_yearly)-0),'-',(DATEPART(MM, month_ppm_case_target_yearly)-0),'-','1')
//     ";
