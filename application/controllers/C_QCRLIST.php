...
class C_QCRLIST extends CI_Controller {
	 ...
	 **/
     ...
     public function __construct(){
        ...
        $this->load->model('M_QCRLIST');//untuk mengoneksi ke tanggal masuk model QCRLIST
	...
	{
        // untuk select filter
        $data['date'] = ['24-06-2022','13-07-20022','17-08-2000']; //untuk select filter date
        $data['pcn_number'] = ['PO01','PO02','PO03']; //untuk select filter part number
        $data['supplier'] = ['ADIT','TONY','JOHNNY']; //untuk select filter part name

        

        // // $data['QCRLIST'] = $this->M_QCRLIST->Tampil_Data();
        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar'); //Tampil Sidebar
		// // $this->load->view('QCRLIST/V_QCRLIST',$data); // Tampil data
        $this->load->view('QCRLIST/V_QCRLIST',$data); // Tampil data
     ...
     function view_data()
     {
      ...
      $tables = "tb_QCRLIST";//untuk data bisa masuk ke table QCR LIST ke database
     $search = array('hdrid','pcn_number','supplier','supplier2','pcn_number2','part_number','product_group','reason_of_change','date_of_request','actual_finish_date','n5_document_dimension','n5_document_performance','capability_data_dimension','capability_data_perfomance','air_leak_data');//untuk data table QCR LIST bisa masuk sesuai input      

         // jika memakai IS NULL pada where sql
         $isWhere = null;
         // $isWhere = 'artikel.deleted_at IS NULL';
         header('Content-Type: application/json');
         echo $this->M_QCRLIST->get_tables($tables,$search,$isWhere);//untuk data table QCRLIST controller connect ke model
   ...
   ///@see get view data where
    ...
    function view_data_where()
    {
      ...
      $search = array('hdrid','pcn_number','supplier','supplier2','pcn_number2','part_number','product_group','reason_of_change','date_of_request','actual_finish_date','n5_document_dimension','n5_document_performance','capability_data_dimension','capability_data_perfomance','air_leak_data');//untuk data table QCR LIST bisa masuk sesuai input
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_QCRLIST->get_tables_where($tables,$search,$where,$isWhere);//untuk data table QCRLIST controller connect ke model
    ...
    function view_data_query()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_Datatables->get_tables_query($query,$search,$where,$isWhere); //untuk data table QCRLIST controller connect ke model
    ...
    ///@see get add data
     ...
     public function ajax_add()
     {
         ...
         $hdrid2 = $this->M_QCRLIST->Max_data($kode, 'tb_qcrlist')->row();//untuk max data
        ...
        $this->M_QCRLIST->Input_Data($post_datamerge,'tb_QCRLIST');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['n5_document_dimension']['name']))//jika document sudah upload maka document bisa ditampilkan
        {
          $this->upload_file_attach('n5_document_dimension',$hdrid,'tb_PCNLIST');//jika upload data dengan attach file sesual file maka data sudah masuk
        }
 if(!empty($_FILES['n5_document_performance']['name']))//jika document sudah upload maka document bisa ditampilkan
        {
          $this->upload_file_attach('n5_document_performance',$hdrid,'tb_PCNLIST');//jika upload data dengan attach file sesual file maka data sudah masuk
        }
 if(!empty($_FILES['capability_data_dimension']['name']))//jika document sudah upload maka document bisa ditampilkan
        {
          $this->upload_file_attach('capability_data_dimension',$hdrid,'tb_PCNLIST');//jika upload data dengan attach file sesual file maka data sudah masuk
        }
 if(!empty($_FILES['capability_data_perfomance']['name']))//jika document sudah upload maka document bisa ditampilkan
        {
          $this->upload_file_attach('capability_data_perfomance',$hdrid,'tb_PCNLIST');//jika upload data dengan attach file sesual file maka data sudah masuk
        }
 if(!empty($_FILES['air_leak_data']['name']))//jika document sudah upload maka document bisa ditampilkan
        {
          $this->upload_file_attach('air_leak_data',$hdrid,'tb_PCNLIST');//jika upload data dengan attach file sesual file maka data sudah masuk
        }
 if(!empty($_FILES['new_dies_machine']['name']))//jika document sudah upload maka document bisa ditampilkan
        {
          $this->upload_file_attach('new_dies_machine',$hdrid,'tb_PCNLIST');//jika upload data dengan attach file sesual file maka data sudah masuk
        }
 if(!empty($_FILES['another_document']['name']))//jika document sudah upload maka document bisa ditampilkan
        {
          $this->upload_file_attach('another_document',$hdrid,'tb_PCNLIST');//jika upload data dengan attach file sesual file maka data sudah masuk
        }
 if(!empty($_FILES['another_document_2']['name']))//jika document sudah upload maka document bisa ditampilkan
        {
          $this->upload_file_attach('another_document_2',$hdrid,'tb_PCNLIST');//jika upload data dengan attach file sesual file maka data sudah masuk
        } 
 ...
 if(!empty($_FILES['another_document_2']['name']))//jika document sudah upload maka document bisa ditampilkan
        ...
        $this->M_QCRLIST->Update_Data($where,$post_datamerge,'tb_QCRLIST');//untuk merge data sudah diupdate
   ...
   ///@see get delete
    ...
    public function ajax_delete()
	{
        ...
        $this->M_QCRLIST->Delete_Data($where,'tb_QCRLIST');//untuk delete table QCRLIST
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_QCRLIST->ajax_getbyhdrid($hdrid,'tb_QCRLIST')->row();//untuk request auto increnete
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_QCRLIST->get_approver($id_user_reg); //untuk request approve
		$data['get_requester']=$this->M_QCRLIST->get_requester($id_user_reg); //untuk request auto increnete
		$data['data']=$this->M_QCRLIST->get_data($id_user_reg); //untuk request nomor pcn
        
        $this->load->view('email/V_QCRLIST',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))//jika data bisa attach file
        {
            ...
            $config['upload_path'] = './assets/upload/QCRLISTlist/';   //path file
            ...
            if ( ! $this->upload->do_upload($filename)){//untuk mengecek status attach file jika sudah masuk
                ...
                $this->M_QCRLIST->Update_Data($where,$data_update,$table);
    ...
    public function import() {
        ...
        $objPHPExcel = PHPExcel_IOFactory::load($inputFileName); //excel path
                     ...
                     // ********************* 0. Generate nomor transaksi  *********************    
                      ...
                      if ($index > 1 && ucwords($value['C'])!='' ) { //diambil dari baris ke 2
                            ...
                            if($index==3){
                                ...
                                $hdrid2=$this->M_QCRLIST->Max_data($mdate,'tb_QCRLIST')->row();  
                                if ($hdrid2->hdrid==NULL){
                                    // Jika belum ada
                                $hdrid3=$mdate."001";
                        ...
                        // ---------------------------------- Import Macro Batas sini 1---------------------------------
                            ...
                            $resultData[$index]['supplier2'] = ucwords($value['A']);
                            $resultData[$index]['pcn_number2'] = ucwords($value['A']);
                            $resultData[$index]['part_number'] = ucwords($value['A']);
                            $resultData[$index]['product_group'] = ucwords($value['A']);
                            $resultData[$index]['reason_of_change'] = ucwords($value['A']);
                            $resultData[$index]['date_of_request'] = ucwords($value['A']);
                            $resultData[$index]['actual_finish_date'] = ucwords($value['A']);
                            $resultData[$index]['n5_document_dimension'] = ucwords($value['A']);
                            $resultData[$index]['n5_document_performance'] = ucwords($value['A']);
                            $resultData[$index]['capability_data_dimension'] = ucwords($value['A']);
                            $resultData[$index]['capability_data_perfomance'] = ucwords($value['A']);
                            $resultData[$index]['air_leak_data'] = ucwords($value['A']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_QCRLIST->insert_batch('tb_QCRLIST',$resultData);
					if ($result > 0) {
