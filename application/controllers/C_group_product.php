...
class C_group_product extends CI_Controller {
	 ...
	 **/
     ...
     public function __construct(){
        ...
        $this->load->model('M_group_product');
        $this->load->model('UserModel');  //untuk load user model hak akses menu  
        // $this->load->library('encrypt');    

        // Cari hak akses by controller
	    $Hak_akses = $this->UserModel->get_controller_access($this->session->userdata('role_id'),'C_group_product'); 
	...
	public function Index()
	{
        ...
        // // $data['group_product'] = $this->M_group_product->Tampil_Data();
        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar_new',$menu_akses); //Tampil Sidebar
		// // $this->load->view('group_product/V_group_product',$data); // Tampil data
        $this->load->view('group_product/V_group_product'); // Tampil data
     ...
     function view_data()
     {
        ...
        $tables = "tb_group_product";        
        $search = array('hdrid','group_product_name');

         // jika memakai IS NULL pada where sql
         $isWhere = null;
         // $isWhere = 'artikel.deleted_at IS NULL';
         header('Content-Type: application/json');
         echo $this->M_group_product->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_group_product->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_group_product->Max_data($mdate,'tb_group_product')->row();        
        ...
        $this->M_group_product->Input_Data($post_datamerge,'tb_group_product');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_group_product');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_group_product->Update_Data($where,$post_datamerge,'tb_group_product');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_group_product->Delete_Data($where,'tb_group_product');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_group_product->ajax_getbyhdrid($hdrid,'tb_group_product')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_group_product->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_group_product->get_requester($id_user_reg); 
		$data['data']=$this->M_group_product->get_data($id_user_reg); 
        
        $this->load->view('email/V_group_product',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_group_product->Update_Data($where,$data_update,$table);
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				if (count($resultData) != 0) {
					$result = $this->M_group_product->insert_batch('tb_group_product',$resultData);
					if ($result > 0) {
