...
class C_pic extends CI_Controller {
	...
	public function Index()
	{
        ...
        // // $data['pic'] = $this->M_pic->Tampil_Data();
        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar'); //Tampil Sidebar
		// // $this->load->view('pic/V_pic',$data); // Tampil data
        $this->load->view('pic/V_pic'); // Tampil data
     ...
     function view_data()
     {
        ...
        $search = array('hdrid','name');
         ...
         echo $this->M_pic->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_pic->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_pic->Max_data($mdate,'tb_pic')->row();        
        ...
        $this->M_pic->Input_Data($post_datamerge,'tb_pic');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_pic');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_pic->Update_Data($where,$post_datamerge,'tb_pic');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_pic->Delete_Data($where,'tb_pic');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_pic->ajax_getbyhdrid($hdrid,'tb_pic')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_pic->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_pic->get_requester($id_user_reg); 
		$data['data']=$this->M_pic->get_data($id_user_reg); 
        
        $this->load->view('email/V_pic',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_pic->Update_Data($where,$data_update,$table);
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				if (count($resultData) != 0) {
					$result = $this->M_pic->insert_batch('tb_pic',$resultData);
					if ($result > 0) {
