...
class C_PCN extends CI_Controller
{
  ...
  public function Index()
  {
    ...
    $data['part_number'] = $this->M_PCN->get_part_number();
    $data['part_name'] = $this->M_PCN->get_part_name();
  ...
  function view_data_where()
  {
    ...
    if ($this->session->userdata('rolename') == 'Administrator EPCN' || $this->session->userdata('rolename') == 'Super Admin') {
      $tables = "fn_view_pcn_register('')"; // Terbuka semua tanpa user
      // $tables = "tb_pcn"; // Terbuka semua tanpa user
    } else {
    ...
    if ($_POST['searchByFromdate'] == '' || $_POST['searchByTodate'] == '') { //untuk pilihan date
      ...
      if ($this->session->userdata('rolename') == 'Administrator EPCN') { //Jika sebagai administrator maka akan tampil semua
        ...
        $where = array('transaction_date >' => $_POST['searchByFromdate'], 'transaction_date <' => $_POST['searchByTodate'], 'nik' => $nik); //untuk pilihan date
      }
    }
    ;
  ...
  ///@see get_pcn_data()
  ///@note fungsi digunakan untuk mengambil data dari tabel PCN
  ///@attention
  public function get_pcn_data()
  {
    $this->datatables->select('
        CONCAT("<button>Edit</button><button>Delete</button>") AS action,
        pic_buyer,
        SUM(CASE WHEN status = "Open" THEN 1 ELSE 0 END) AS open,
        SUM(CASE WHEN status = "Not Yet Temporary" THEN 1 ELSE 0 END) AS not_yet_temporary,
        SUM(CASE WHEN status = "Hold" THEN 1 ELSE 0 END) AS hold,
        SUM(CASE WHEN status = "Temporary" THEN 1 ELSE 0 END) AS temporary,
        SUM(CASE WHEN status = "Late" THEN 1 ELSE 0 END) AS late,
        SUM(CASE WHEN priority = "High" THEN 1 ELSE 0 END) AS priority
    ');
    $this->datatables->from('tb_PCN');
    $this->datatables->group_by('pic_buyer');
  ...
  function ajax_send_back()
  {
    ...
    $status = $this->M_PCN->Send_Back_Data($where_pcn, $where, $data, $reason, 'tb_approval', $name);
  ...
  public function ajax_update()
  {
    ...
    if ($this->input->post('stat') == 'Rejected' || substr($this->input->post('stat'), 0, 9) == 'Send Back') {
      $data = array('stat' => 'unapprove');
      $this->M_PCN->Update_Data($where, $data, 'tb_PCN');
    }
    $data['hdrid'] = $hdrid;
  ...
  public function ajax_delete()
  {
    ...
    $where_q = array('reason' => $this->input->post('hdrid'));
    ...
    $this->M_PCN->Delete_Data($where_q, 'tb_qcr'); //untuk delete table PCN
    $data['status'] = "berhasil dihapus"; //jika sudah berhasil dihapus maka data akan kosong
    // return value array
    $this->output->set_content_type('application/json')->set_output(json_encode($data));
