...
class C_sys_set_app extends CI_Controller {
	...
	public function Index()
	{
     ...
     function view_data()
     {
        ...
        $tables = "tb_settingapp";
        $search = array('hdrid','product','pe_nik','pe_pic','pe_name','qc_nik','qc_pic','qc_name','mfg_nik','mfg_pic','mfg_name','pc_nik','pc_pic','pc_name','qa_nik','qa_pic','qa_name');
        // jika memakai IS NULL pada where sql
         $isWhere = null;
         // $isWhere = 'artikel.deleted_at IS NULL';
         header('Content-Type: application/json');
         echo $this->M_sys_set_app->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_sys_set_app->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_sys_set_app->Max_data($mdate,'tb_settingapp')->row();        
        ...
        $this->M_sys_set_app->Input_Data($post_datamerge,'tb_settingapp');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_settingapp');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_sys_set_app->Update_Data($where,$post_datamerge,'tb_settingapp');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_sys_set_app->Delete_Data($where,'tb_settingapp');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_sys_set_app->ajax_getbyhdrid($hdrid,'tb_settingapp')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_sys_set_app->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_sys_set_app->get_requester($id_user_reg); 
		$data['data']=$this->M_sys_set_app->get_data($id_user_reg); 
        
        $this->load->view('email/V_sys_set_app',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_sys_set_app->Update_Data($where,$data_update,$table);
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				foreach ($sheetData as $key => $value) {
                       ...
                       if($rowexcel>2){
                        ...
                        $resultData[$index]['pe_nik'] = ucwords($value['A']);
                        $resultData[$index]['pe_pic'] = ucwords($value['A']);
                        $resultData[$index]['pe_name'] = ucwords($value['A']);
                        $resultData[$index]['qc_nik'] = ucwords($value['A']);
                        $resultData[$index]['qc_pic'] = ucwords($value['A']);
                        $resultData[$index]['qc_name'] = ucwords($value['A']);
                        $resultData[$index]['mfg_nik'] = ucwords($value['A']);
                        $resultData[$index]['mfg_pic'] = ucwords($value['A']);
                        $resultData[$index]['mfg_name'] = ucwords($value['A']);
                        $resultData[$index]['pc_nik'] = ucwords($value['A']);
                        $resultData[$index]['pc_pic'] = ucwords($value['A']);
                        $resultData[$index]['pc_name'] = ucwords($value['A']);
                        $resultData[$index]['qa_nik'] = ucwords($value['A']);
                        $resultData[$index]['qa_pic'] = ucwords($value['A']);
                        $resultData[$index]['qa_name'] = ucwords($value['A']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_sys_set_app->insert_batch('tb_settingapp',$resultData);
					if ($result > 0) {
