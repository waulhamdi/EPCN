...
class C_source_information extends CI_Controller {
	 ...
	 **/
     ...
     public function __construct(){
        ...
        $this->load->model('M_source_information');
	...
	public function Index()
	{
        ...
        // // $data['source_information'] = $this->M_source_information->Tampil_Data();
        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar'); //Tampil Sidebar
		// // $this->load->view('source_information/V_source_information',$data); // Tampil data
        $this->load->view('source_information/V_source_information'); // Tampil data
     ...
     function view_data()
     {
        ...
        $tables = "tb_source_information";        
        $search = array('hdrid','problem','attach_file','source_information_name');

         // jika memakai IS NULL pada where sql
         $isWhere = null;
         // $isWhere = 'artikel.deleted_at IS NULL';
         header('Content-Type: application/json');
         echo $this->M_source_information->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_source_information->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_source_information->Max_data($mdate,'tb_source_information')->row();        
        ...
        $this->M_source_information->Input_Data($post_datamerge,'tb_source_information');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_source_information');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_source_information->Update_Data($where,$post_datamerge,'tb_source_information');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_source_information->Delete_Data($where,'tb_source_information');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_source_information->ajax_getbyhdrid($hdrid,'tb_source_information')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_source_information->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_source_information->get_requester($id_user_reg); 
		$data['data']=$this->M_source_information->get_data($id_user_reg); 
        
        $this->load->view('email/V_source_information',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_source_information->Update_Data($where,$data_update,$table);
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				foreach ($sheetData as $key => $value) {
                     ...
                     // ********************* 0. Generate nomor transaksi  *********************          
                       ...
                       // ---------------------------------- Import Macro Batas sini 1---------------------------------
                        ...
                        $resultData[$index]['attach_file'] = ucwords($value['A']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_source_information->insert_batch('tb_source_information',$resultData);
					if ($result > 0) {
