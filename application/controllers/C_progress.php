...
class C_progress extends CI_Controller
{
    ...
    function view_data_where()
    {
        ...
        $tables = "tb_input_problem";
        $search = array('a.hdrid', 'a.group_product_id', 'a.group_product_name','a.due_date', 'a.customer_id', 'a.customer_name', 'a.problem_name','a.status_transaction');
        
        if ($_POST['searchByFromdate'] == '' || $_POST['searchByTodate'] == '') {

            if($this->session->userdata('rolename')=='Administrator Quality'){
                $where  = array('a.transaction_date >' => '2020-01-01', 'a.draft' => 'Send');
            }else{
            ...
            if($this->session->userdata('rolename')=='Administrator Quality'){
                $where  = array('a.transaction_date >' => $_POST['searchByFromdate'], 'a.transaction_date <' => $_POST['searchByTodate'], 'a.draft' => 'Send');
            }else{
    ...
    function view_data_where_new()
    {
         ...
         if($this->session->userdata('rolename')=='Administrator Quality'){
                ...
                $nikSession = $this->session->userdata('user_name');
                $tables = "fn_Progress('$nikSession')";  
         }
       
        $search = array('hdrid','due_date','status_transaction','group_product_name','problem_name','customer_name','name2');
    ...
    public function ajax_save_responsible()
    {
        ...
        $status_transaction= $this->M_progress->Update_Data_Respone_ver2($hdrid,mdate('%Y-%m-%d %H:%i:%s', time()));
    ...
    public function ajax_update_containment_date_actual()
    {

        // ********************* 1. Collect data post *********************
        date_default_timezone_set('Asia/Jakarta');
        $post_data = array('containment_date_actual' => mdate('%Y-%m-%d', time()));
        $hdrid = $this->input->post('hdrid');
    ...
    public function ajax_updateres()
    {
        ...
        if(!empty($_FILES['attach_file_occurence']['name']))
        {
            $this->upload_file_attach2('attach_file_occurence',$hdrid,'tb_response');
        }
        ...
        // **********************  Upload file investigation_attach  *********************   
        if (!empty($_FILES['investigation_attach']['name'])) {
            $this->upload_file_attach2('investigation_attach', $hdrid, 'tb_response');
        }
    ...
    function ajax_approve()
    {
        ...
        $status_transaction=$this->M_approver->Update_Data_Approve_ver2($where, $data2, 'tb_approval',$problem_id);
