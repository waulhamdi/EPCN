...
class C_monitoring_problem extends CI_Controller {
	 ...
	 **/
     ...
     public function __construct(){
        ...
        $this->load->model('M_monitoring_problem');
	...
	public function Index()
	{
        ...
        // $data['groups'] = $this->M_monitoring_problem->get_group_monitoring_problem();
        // var_dump($data);
        // die;

        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar'); //Tampil Sidebar
		// // $this->load->view('monitoring_problem/V_monitoring_problem',$data); // Tampil data
        $data['ranks'] = ['S','A','B','C'];
        $this->load->view('monitoring_problem/V_monitoring_problem',$data); // Tampil data
     ...
     function view_data()
     {
        ...
        $search = array('hdrid','problem_category_id','problem_category_name','group_product_id','group_product_name','product_id','product_name','customer_id','customer_name','contact_person_name','source_information_id','source_information_name','customer_report_number','part_number','qty','lot_number_product','problem','detail_problem','responsible_section','containment_date','nik','name','section1','email1','nik2','name2','section2','email2','report_date','status','problem_name','due_date','issue_date','rank_problem','attach_picture','temporary_action','etc','delivery_status','when_problem','shift_problem','time_problem','who_problem','where_problem','customer_product_id','qty_lot','comment','rank_problem','status_transaction');
         ...
         echo $this->M_monitoring_problem->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        //$tables = "tb_input_problem";    
        $tables = "fn_Progress('')";    
        $search = array('hdrid','problem_category_id','problem_category_name','group_product_id','group_product_name','product_id','product_name','customer_id','customer_name','contact_person_name','source_information_id','source_information_name','customer_report_number','part_number','qty','lot_number_product','problem','detail_problem','responsible_section','containment_date','nik','name','section1','email1','nik2','name2','section2','email2','report_date','status','problem_name','due_date','issue_date','rank_problem','attach_picture','temporary_action','etc','delivery_status','when_problem','shift_problem','time_problem','who_problem','where_problem','customer_product_id','qty_lot','comment','rank_problem','status_transaction','Status_monitoring');
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_monitoring_problem->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_monitoring_problem->Max_data($mdate,'tb_input_problem')->row();        
        ...
        $this->M_monitoring_problem->Input_Data($post_datamerge,'tb_input_problem');
    ...
    public function ajax_update()
	{
        ...
        $this->M_monitoring_problem->Update_Data($where,$post_datamerge,'tb_input_problem');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_monitoring_problem->Delete_Data($where,'tb_input_problem');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_monitoring_problem->ajax_getbyhdrid($hdrid,'tb_input_problem')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_monitoring_problem->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_monitoring_problem->get_requester($id_user_reg); 
		$data['data']=$this->M_monitoring_problem->get_data($id_user_reg); 
        
        $this->load->view('email/V_monitoring_problem',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_monitoring_problem->Update_Data($where,$data_update,$table);
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				foreach ($sheetData as $key => $value) {
                     ...
                     // ********************* 0. Generate nomor transaksi  *********************          
                       ...
                       // ---------------------------------- Import Macro Batas sini 1---------------------------------
                        ...
                        $resultData[$index]['monitoring_problem_name'] = ucwords($value['A']);
                        $resultData[$index]['report_no'] = ucwords($value['A']);
                        $resultData[$index]['group_monitoring_problem_id'] = ucwords($value['A']);
                        $resultData[$index]['group_monitoring_problem_name'] = ucwords($value['A']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_monitoring_problem->insert_batch('tb_input_problem',$resultData);
					if ($result > 0) {
