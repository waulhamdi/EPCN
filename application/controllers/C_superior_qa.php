...
class C_superior_qa extends CI_Controller {
	 ...
	 **/
     ...
     public function __construct(){
        ...
        $this->load->model('M_superior_qa');
        $this->load->model('UserModel');  //untuk load user model hak akses menu    
        // $this->load->library('encrypt');    

        // Cari hak akses by controller
	    $Hak_akses = $this->UserModel->get_controller_access($this->session->userdata('role_id'),'C_superior_qa'); 
	...
	public function Index()
	{
        ...
        $data['nik'] = $this->M_superior_qa->Tampil_user();
        $data['nik_superior_qa'] = $this->M_superior_qa->Tampil_user();
		...
		// // $this->load->view('superior_proc/V_superior_qa',$data); // Tampil data
        $this->load->view('superior_qa/V_superior_qa',$data); // Tampil data
     ...
     function view_data()
     {
        ...
        $tables = "tb_superiorqa";
        $search = array('hdrid','nik_superiorqa','name_superiorqa','kode_section_superiorqa','name_section_superiorqa','email_superiorqa','product','position');


         $isWhere = null;
         // $isWhere = 'artikel.deleted_at IS NULL';
         header('Content-Type: application/json');
         echo $this->M_superior_qa->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_superior_qa->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $mdate="K";        
        $hdrid2=$this->M_superior_qa->Max_data($mdate,'tb_superiorqa')->row();        
        ...
        $this->M_superior_qa->Input_Data($post_datamerge,'tb_superiorqa');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_superiorqa');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_superior_qa->Update_Data($where,$post_datamerge,'tb_superiorqa');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_superior_qa->Delete_Data($where,'tb_superiorqa');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_superior_qa->ajax_getbyhdrid($hdrid,'tb_superiorqa')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_superior_qa->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_superior_qa->get_requester($id_user_reg); 
		$data['data']=$this->M_superior_qa->get_data($id_user_reg); 
        
        $this->load->view('email/V_superior_qa',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_superior_qa->Update_Data($where,$data_update,$table);
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				foreach ($sheetData as $key => $value) {
                       ...
                       if($rowexcel>2){
                        ...
                        // ---------------------------------- Import Macro Batas sini 1---------------------------------
                            ...
                            $resultData[$index]['nik_superiorqa'] = ucwords($value['A']);
                            $resultData[$index]['name_superiorqa'] = ucwords($value['A']);
                            $resultData[$index]['kode_section_superiorqa'] = ucwords($value['A']);
                            $resultData[$index]['name_section_superiorqa'] = ucwords($value['A']);
                            $resultData[$index]['email_superiorqa'] = ucwords($value['A']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_superior_qa->insert_batch('tb_superiorqa',$resultData);
					if ($result > 0) {
