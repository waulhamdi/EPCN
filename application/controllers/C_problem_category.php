...
class C_problem_category extends CI_Controller {
	 ...
	 **/
     ...
     public function __construct(){
        ...
        $this->load->model('M_problem_category');
	...
	public function Index()
	{
        ...
        // // $data['problem_category'] = $this->M_problem_category->Tampil_Data();
        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar'); //Tampil Sidebar
		// // $this->load->view('problem_category/V_problem_category',$data); // Tampil data
        $this->load->view('problem_category/V_problem_category'); // Tampil data
     ...
     function view_data()
     {
        ...
        $tables = "tb_problem_category";        
        $search = array('hdrid','problem_category_name');

         // jika memakai IS NULL pada where sql
         $isWhere = null;
         // $isWhere = 'artikel.deleted_at IS NULL';
         header('Content-Type: application/json');
         echo $this->M_problem_category->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_problem_category->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_problem_category->Max_data($mdate,'tb_problem_category')->row();        
        ...
        $this->M_problem_category->Input_Data($post_datamerge,'tb_problem_category');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_problem_category');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_problem_category->Update_Data($where,$post_datamerge,'tb_problem_category');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_problem_category->Delete_Data($where,'tb_problem_category');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_problem_category->ajax_getbyhdrid($hdrid,'tb_problem_category')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_problem_category->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_problem_category->get_requester($id_user_reg); 
		$data['data']=$this->M_problem_category->get_data($id_user_reg); 
        
        $this->load->view('email/V_problem_category',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_problem_category->Update_Data($where,$data_update,$table);
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				if (count($resultData) != 0) {
					$result = $this->M_problem_category->insert_batch('tb_problem_category',$resultData);
					if ($result > 0) {
