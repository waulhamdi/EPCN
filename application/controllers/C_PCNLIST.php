...
class C_PCNLIST extends CI_Controller
{
  ...
  public function view_data_where()
  {
    ...
    $search = array('transaction_date', 'no_dokumen', 'status', 'category', 'supplier_name', 'product_name', 'part_name', 'part_no', 'description', 'proses_perubahan_lama', 'proses_perubahan_baru', 'current_flow_pic', 'pic_proc', 'commodity', 'qa_pic', 'registered', 'masspro_schedule', 'isir', 'qcr', 'report_pe'); //untuk data table pcnlist bisa masuk sesuai input
    ...
    if ($_POST['searchByFromdate'] == '' || $_POST['searchByTodate'] == '') { //Jika sebagai administrator maka akan tampil semua
      $where = array('transaction_date >' => '1900-01-01'); //menunjukkan tanggal transaksi             
    } else {
    ...
    // $isWhere = 'artikel.deleted_at IS NULL';
    header('Content-Type: application/json');
    echo $this->M_PCN->get_tables_where($tables, $search, $where, $isWhere); //untuk data table pcnlist controller connect ke model
  ...
  public function ajax_delete()
  {
    ...
    $this->M_PCN->Delete_Data($where1, 'tb_isir'); //untuk delete table PCN
    $this->M_PCN->Delete_Data($where1, 'tb_isir_list'); //untuk delete table PCN
