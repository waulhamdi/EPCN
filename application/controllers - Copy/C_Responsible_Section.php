...
class C_responsible_section extends CI_Controller {
	 ...
	 **/
     ...
     public function __construct(){
        ...
        $this->load->model('M_responsible_section');
	...
	public function Index()
	{
        ...
        // // $data['responsible_section'] = $this->M_responsible_section->Tampil_Data();
        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar'); //Tampil Sidebar
		// // $this->load->view('responsible_section/V_responsible_section',$data); // Tampil data
        $this->load->view('responsible_section/V_responsible_section'); // Tampil data
     ...
     function view_data()
     {
        ...
        $tables = "tb_responsible_section";        
        $search = array('hdrid','name');
         // jika memakai IS NULL pada where sql
         $isWhere = null;
         // $isWhere = 'artikel.deleted_at IS NULL';
         header('Content-Type: application/json');
         echo $this->M_responsible_section->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_responsible_section->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_responsible_section->Max_data($mdate,'tb_responsible_section')->row();        
        ...
        $this->M_responsible_section->Input_Data($post_datamerge,'tb_responsible_section');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_responsible_section');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_responsible_section->Update_Data($where,$post_datamerge,'tb_responsible_section');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_responsible_section->Delete_Data($where,'tb_responsible_section');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_responsible_section->ajax_getbyhdrid($hdrid,'tb_responsible_section')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_responsible_section->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_responsible_section->get_requester($id_user_reg); 
		$data['data']=$this->M_responsible_section->get_data($id_user_reg); 
        
        $this->load->view('email/V_responsible_section',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_responsible_section->Update_Data($where,$data_update,$table);
    ...
    public function import() {
		...
		if ($_FILES['excel']['name'] == '') {
            ...
            redirect('C_responsible_section');
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				if (count($resultData) != 0) {
					$result = $this->M_responsible_section->insert_batch('tb_responsible_section',$resultData);
					if ($result > 0) {
