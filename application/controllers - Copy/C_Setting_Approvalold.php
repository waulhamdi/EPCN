...
class C_setting_approval_old extends CI_Controller {
	 ...
	 **/
     ...
     public function __construct(){
        ...
        $this->load->model('M_setting_approval_old');
	...
	public function Index()
	{
        ...
        $data['group'] = $this->M_setting_approval_old->tampil_group();
        $data['nik'] = $this->M_setting_approval_old->tampil_nik();
        $data['department'] = $this->M_setting_approval_old->tampil_department();
        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar'); //Tampil Sidebar
		// // $this->load->view('setting_approval_old/V_setting_approval_old',$data); // Tampil data
        $this->load->view('setting_approval_old/V_setting_approval_old'); // Tampil data
     ...
     function view_data()
     {
        ...
        $tables = "tb_setting_approval_old";        
        $search = array('hdrid','group_id','group_name','nik','name','email_address','department_code','department_name','position');

         // jika memakai IS NULL pada where sql
         $isWhere = null;
         // $isWhere = 'artikel.deleted_at IS NULL';
         header('Content-Type: application/json');
         echo $this->M_setting_approval_old->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_setting_approval_old->get_tables_where($tables,$search,$where,$isWhere);
    ...
    function view_data_query()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_setting_approval_old->get_tables_query($query,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_setting_approval_old->Max_data($mdate,'tb_setting_approval_old')->row();        
        ...
        $this->M_setting_approval_old->Input_Data($post_datamerge,'tb_setting_approval_old');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_setting_approval_old');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_setting_approval_old->Update_Data($where,$post_datamerge,'tb_setting_approval_old');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_setting_approval_old->Delete_Data($where,'tb_setting_approval_old');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_setting_approval_old->ajax_getbyhdrid($hdrid,'tb_setting_approval_old')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_setting_approval_old->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_setting_approval_old->get_requester($id_user_reg); 
		$data['data']=$this->M_setting_approval_old->get_data($id_user_reg); 
        
        $this->load->view('email/V_setting_approval_old',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_setting_approval_old->Update_Data($where,$data_update,$table);
    ...
    public function import() {
		...
		if ($_FILES['excel']['name'] == '') {
            ...
            redirect('C_setting_approval_old');
				...
				foreach ($sheetData as $key => $value) {
                     ...
                     // ********************* 0. Generate nomor transaksi  *********************          
                       ...
                       // ---------------------------------- Import Macro Batas sini 1---------------------------------
                        ...
                        $resultData[$index]['group_id'] = ucwords($value['A']);
$resultData[$index]['group_name'] = ucwords($value['A']);
$resultData[$index]['nik'] = ucwords($value['A']);
$resultData[$index]['name'] = ucwords($value['A']);
$resultData[$index]['email_address'] = ucwords($value['A']);
$resultData[$index]['department_code'] = ucwords($value['A']);
...
$resultData[$index]['position'] = ucwords($value['A']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_setting_approval_old->insert_batch('tb_setting_approval_old',$resultData);
					if ($result > 0) {
