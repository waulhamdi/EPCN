...
class C_internal extends CI_Controller {
	...
	public function Index()
	{
        ...
        // // $data['internal'] = $this->M_internal->Tampil_Data();
        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar'); //Tampil Sidebar
		// // $this->load->view('internal/V_internal',$data); // Tampil data
        $this->load->view('internal/V_internal'); // Tampil data
     ...
     function view_data()
     {
        ...
        $tables = "tb_internal";        
        $search = array('hdrid','internal_name');

         // jika memakai IS NULL pada where sql
         $isWhere = null;
         // $isWhere = 'artikel.deleted_at IS NULL';
         header('Content-Type: application/json');
         echo $this->M_internal->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_internal->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_internal->Max_data($mdate,'tb_internal')->row();        
        ...
        $this->M_internal->Input_Data($post_datamerge,'tb_internal');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_internal');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_internal->Update_Data($where,$post_datamerge,'tb_internal');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_internal->Delete_Data($where,'tb_internal');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_internal->ajax_getbyhdrid($hdrid,'tb_internal')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_internal->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_internal->get_requester($id_user_reg); 
		$data['data']=$this->M_internal->get_data($id_user_reg); 
        
        $this->load->view('email/V_internal',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_internal->Update_Data($where,$data_update,$table);
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				if (count($resultData) != 0) {
					$result = $this->M_internal->insert_batch('tb_internal',$resultData);
					if ($result > 0) {
