...
class C_Customer_Name extends CI_Controller {
	...
	public function Index()
	{
     ...
     function view_data()
     {
        ...
        $tables = "tb_customer_name";        
        $search = array('hdrid','customer_name');
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_Customer_Name->Max_data($mdate,'tb_Customer_Name')->row();        
        ...
        $this->M_Customer_Name->Input_Data($post_datamerge,'tb_Customer_Name');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_Customer_Name');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_Customer_Name->Update_Data($where,$post_datamerge,'tb_Customer_Name');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_Customer_Name->Delete_Data($where,'tb_Customer_Name');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_Customer_Name->ajax_getbyhdrid($hdrid,'tb_Customer_Name')->row();
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				if (count($resultData) != 0) {
					$result = $this->M_Customer_Name->insert_batch('tb_Customer_Name',$resultData);
					if ($result > 0) {
