...
class C_ppm_case_target extends CI_Controller {
	 ...
	 **/
     ...
     public function __construct(){
        ...
        $this->load->model('M_ppm_case_target');
	...
	{
        $data['group_products'] = $this->M_ppm_case_target->tampil_group_product();
        $data['jeniss'] = ['PPM','Case'];
        // $dt['ppm_case_date'] = $this->M_ppm_case_target->get_ppm_case_date();
        // var_dump($dt);

        // // $data['ppm_case_target'] = $this->M_ppm_case_target->Tampil_Data();
        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar'); //Tampil Sidebar
		// // $this->load->view('ppm_case_target/V_ppm_case_target',$data); // Tampil data
        $this->load->view('ppm_case_target/V_ppm_case_target',$data); // Tampil data
     ...
     function view_data()
     {
        ...
        $tables = "tb_ppm_case_target";      
        $search = array('hdrid','year_ppm_case_target','month_ppm_case_target','case_target','case_actual','ppm_target','ppm_actual','ppm_case_date','group_product_id','group_product_name','quantity_ppm_case_target','jenis','target_monthly','target_yearly');

         // jika memakai IS NULL pada where sql
         $isWhere = null;
         // $isWhere = 'artikel.deleted_at IS NULL';
         header('Content-Type: application/json');
         echo $this->M_ppm_case_target->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_ppm_case_target->get_tables_where($tables,$search,$where,$isWhere);
	...
	{
        // $kodeB =  $this->M_ppm_case_target->get_ppm_case_date();
        
        // ********************* 0. Generate nomor transaksi  *********************          
        $mdate="TR".mdate('%Y%m',time());        
        $hdrid2=$this->M_ppm_case_target->Max_data($mdate,'tb_ppm_case_target')->row();        
        // $ppm_case_date1=$this->M_ppm_case_target->Max_data($mdate,'tb_ppm_case_target')->row();        
        ...
        // $post_data4=$this->M_ppm_case_target->get_ppm_case_date()->row();       
        // $Year=mdate('%Y',$this->input->post('year_ppm_case_target'));
        // $Month=mdate('%m',$this->input->post('month_ppm_case_target'));
        // $Year_Month=$Year ."-".$Month."-"."01";
        // $post_data4=array('ppm_case_date'=>$Year_Month);

        $Year=$this->input->post('year_ppm_case_target');
        $Month=$this->input->post('month_ppm_case_target');
        $Year_Month=$Year ."-".$Month."-"."01";
        $post_data4=array('ppm_case_date'=>$Year_Month);
        ...
        $this->M_ppm_case_target->Input_Data($post_datamerge,'tb_ppm_case_target');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_ppm_case_target');           
        }
    ...
    public function ajax_update()
	{
        ...
        // *********************  Merge data All post *********************
        // $Year=mdate('%Y',$this->input->post('year_ppm_case_target'));
        // $Month=mdate('%Y',$this->input->post('month_ppm_case_target'));
        // $Year_Month=$Year ."-".$Month."01";
        // $post_data4=array('ppm_case_date'=>$Year_Month);
        ...
        $this->M_ppm_case_target->Update_Data($where,$post_datamerge,'tb_ppm_case_target');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_ppm_case_target->Delete_Data($where,'tb_ppm_case_target');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_ppm_case_target->ajax_getbyhdrid($hdrid,'tb_ppm_case_target')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_ppm_case_target->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_ppm_case_target->get_requester($id_user_reg); 
		$data['data']=$this->M_ppm_case_target->get_data($id_user_reg); 
        
        $this->load->view('email/V_ppm_case_target',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_ppm_case_target->Update_Data($where,$data_update,$table);
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				foreach ($sheetData as $key => $value) {
                     ...
                     // ********************* 0. Generate nomor transaksi  *********************          
                       ...
                       // ---------------------------------- Import Macro Batas sini 1---------------------------------
                        ...
                        $resultData[$index]['year_ppm_case_target'] = ucwords($value['A']);
                        // $resultData[$index]['month_ppm_case_target'] = ucwords($value['A']);
                        $resultData[$index]['case_target'] = ucwords($value['A']);
                        $resultData[$index]['case_actual'] = ucwords($value['A']);
                        $resultData[$index]['ppm_target'] = ucwords($value['A']);
                        $resultData[$index]['ppm_actual'] = ucwords($value['A']);
                        $resultData[$index]['ppm_case_date'] = ucwords($value['A']);
                        $resultData[$index]['group_product_id'] = ucwords($value['A']);
                        $resultData[$index]['group_product_name'] = ucwords($value['A']);
                        $resultData[$index]['quantity_ppm_case_target'] = ucwords($value['A']);
                        $resultData[$index]['jenis'] = ucwords($value['A']);
                        $resultData[$index]['target_monthly'] = ucwords($value['A']);
                        $resultData[$index]['target_yearly'] = ucwords($value['A']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_ppm_case_target->insert_batch('tb_ppm_case_target',$resultData);
					if ($result > 0) {
