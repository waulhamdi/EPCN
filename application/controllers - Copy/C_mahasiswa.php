...
class C_mahasiswa extends CI_Controller {
	...
	public function Index()
	{
        ...
        $this->load->view('mahasiswa/V_mahasiswa'); // Tampil data
     ...
     function view_data()
     {
        ...
        $tables = "tb_mahasiswa";  
        $search = array('hdrid','nama','nim','jurusan','jenis_kelamin','tanggal_lahir','ktp');
         ...
         echo $this->M_mahasiswa->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
         ...
         $tables = "tb_mahasiswa";     
        $search = array('hdrid','nama','nim','jurusan','jenis_kelamnin','tanggal_lahir','ktp');
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_mahasiswa->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_mahasiswa->Max_data($mdate,'tb_mahasiswa')->row();        
        ...
        $this->M_mahasiswa->Input_Data($post_datamerge,'tb_mahasiswa');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_mahasiswa');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_mahasiswa->Update_Data($where,$post_datamerge,'tb_mahasiswa');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_mahasiswa->ajax_getbyhdrid($hdrid,'tb_mahasiswa')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_mahasiswa->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_mahasiswa->get_requester($id_user_reg); 
		$data['data']=$this->M_mahasiswa->get_data($id_user_reg); 
        
        $this->load->view('email/V_mahasiswa',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_mahasiswa->Update_Data($where,$data_update,$table);
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
                     ...
                     // ********************* 0. Generate nomor transaksi  *********************          
                       ...
                       // ---------------------------------- Import Macro Batas sini 1---------------------------------
                        ...
                        //  Sample
                            ...
                            $resultData[$index]['nama'] = ucwords($value['A']);
                            $resultData[$index]['nim'] = ucwords($value['A']);
                            $resultData[$index]['jurusan'] = ucwords($value['A']);
                            $resultData[$index]['jenis_kelamin'] = ucwords($value['A']);
                            $resultData[$index]['tanggal_lahir'] = ucwords($value['A']);
                            $resultData[$index]['ktp'] = ucwords($value['A']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_mahasiswa->insert_batch('tb_mahasiswa',$resultData);
					if ($result > 0) {
