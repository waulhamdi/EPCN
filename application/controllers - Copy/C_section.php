...
class C_section extends CI_Controller {
	...
	public function Index()
	{
        ...
        // // $data['section'] = $this->M_section->Tampil_Data();
        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar'); //Tampil Sidebar
		// // $this->load->view('section/V_section',$data); // Tampil data
        $this->load->view('section/V_section'); // Tampil data
     ...
     function view_data()
     {
        ...
        $tables = "tb_section";        
       $search = array('hdrid','section_code','section_name');

         // jika memakai IS NULL pada where sql
         $isWhere = null;
         // $isWhere = 'artikel.deleted_at IS NULL';
         header('Content-Type: application/json');
         echo $this->M_section->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
       ...
       $search = array('hdrid','section_code','section_name');
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_section->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_section->Max_data($mdate,'tb_section')->row();        
        ...
        $this->M_section->Input_Data($post_datamerge,'tb_section');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_section');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_section->Update_Data($where,$post_datamerge,'tb_section');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_section->Delete_Data($where,'tb_section');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_section->ajax_getbyhdrid($hdrid,'tb_section')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_section->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_section->get_requester($id_user_reg); 
		$data['data']=$this->M_section->get_data($id_user_reg); 
        
        $this->load->view('email/V_section',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_section->Update_Data($where,$data_update,$table);
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				foreach ($sheetData as $key => $value) {
                     ...
                     // ********************* 0. Generate nomor transaksi  *********************          
                       ...
                       // ---------------------------------- Import Macro Batas sini 1---------------------------------
                        ...
                        $resultData[$index]['section_code'] = ucwords($value['A']);
                        $resultData[$index]['section_name'] = ucwords($value['A']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_section->insert_batch('tb_section',$resultData);
					if ($result > 0) {
