...
class C_setting_approval_new extends CI_Controller {
	 ...
	 **/
     ...
     public function __construct(){
        ...
        $this->load->model('M_setting_approval_new');
	...
	public function Index()
	{
        ...
        $data['group'] = $this->M_setting_approval_new->Tampil_group();
        $data['nik'] = $this->M_setting_approval_new->Tampil_nik();
        $data['department'] = $this->M_setting_approval_new->Tampil_department();
        $data['position'] = $this->M_setting_approval_new->Tampil_position();
        $data['problem_category'] = $this->M_setting_approval_new->tampil_problem_category();
        
        
        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar'); //Tampil Sidebar
		// // $this->load->view('setting_approval/V_setting_approval_new',$data); // Tampil data
        $this->load->view('setting_approval/V_setting_approval_new', $data); // Tampil data
     ...
     function view_data()
     {
        ...
        $search = array('hdrid','group_id','group_product','problem_category_id','problem_category_name','nik','name','office_email','department_code','department_name','title','position_code','position_name');
         ...
         echo $this->M_setting_approval_new->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_setting_approval_new->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_setting_approval_new->Max_data($mdate,'tb_setting_approval')->row();        
        ...
        $this->M_setting_approval_new->Input_Data($post_datamerge,'tb_setting_approval');
    ...
    public function ajax_update()
	{
        ...
        $this->M_setting_approval_new->Update_Data($where,$post_datamerge,'tb_setting_approval');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_setting_approval_new->Delete_Data($where,'tb_setting_approval');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_setting_approval_new->ajax_getbyhdrid($hdrid,'tb_setting_approval')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_setting_approval_new->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_setting_approval_new->get_requester($id_user_reg); 
		$data['data']=$this->M_setting_approval_new->get_data($id_user_reg); 
        
        $this->load->view('email/V_setting_approval_new',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_setting_approval_new->Update_Data($where,$data_update,$table);
    ...
    public function import() {
		...
		if ($_FILES['excel']['name'] == '') {
            ...
            redirect('C_setting_approval_new');
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				if (count($resultData) != 0) {
					$result = $this->M_setting_approval_new->insert_batch('tb_setting_approval',$resultData);
					if ($result > 0) {
