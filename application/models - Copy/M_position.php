...
class M_position extends CI_Model {
   ...
   {
      $this->db->select_max('position_code');     
      $this->db->like('position_code', $mdate);
   ...
   function ajax_getbyposition_code($position_code,$table){      
      return $this->db->get_where($table, array('position_code' => $position_code));
   }
   ...
   public function Send_mail($position_code)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM position_approver
     where position_code='WTR2010009' and level_approve=(select min(level_approve) from position_approver where position_code='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       $where = array('position_code' => $query->position_code); 
       $data = array('progress_approver' =>'Waiting approved '.$query->name_approver);       
       $this->db->where($where);
       $this->db->update('position_approver',$data);

      // Kirim email request to approve
      $this->db->query("CALL sp_send_mail @position_code='$query->position_code',@nik='$query->nik_approver',@name='$query->name_approver',@email='widodo.a5v@ap.denso.com',@EmailSubject='Waiting Approver Wire transfer No ',@Email_body_content='You need approver'");
     ...
     $this->db->query("exec sp_send_mail @position_code='123',@nik='DM1902060',@name='Widodo',@email='widodo.a5v@ap.denso.com',@EmailSubject='Finish Approver Wire transfer No ',@Email_body_content='You need approver'");
   
    }
    
   }

   public function Send_mail_reject($position_code)
   {
    
      // Cari data wire
      $query =$this->db->query("select position_code,nik,name_requester,isnull(reject_by,'')reject_by,isnull(reason_reject,'')reason_reject from tb_wiretransfer where position_code='$position_code' ");
      $query=$query->row();
      ...
      if ($query2->num_rows()>0) {
          $query2=$query2->row();
         //  Kirim email
          $result =$this->db->query("exec sp_send_mail @position_code='$query->position_code',@nik='$query->nik',@name='$query->name_requester',@email='widodo.a5v@ap.denso.com',@EmailSubject='Rejected Wire transfer No ',@Email_body_content='Rejected by <b> $query->reject_by </b>',@comment='With reason = <b> $query->reason_reject </b>'")->result_array();
      }
