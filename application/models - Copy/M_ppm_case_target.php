...
class M_ppm_case_target extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM ppm_case_target_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from ppm_case_target_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('ppm_case_target_approver',$data);
...
//     $sql="UPDATE tb_ppm_case_target set ppm_case_date = Concat((DATEPART(YYYY, year_ppm_case_target)-0),'-',(DATEPART(MM, month_ppm_case_target)-0),'-','1')
//     ";
